...
function fun_show_hand_to_single_player(v,cards){
    ...
    //console.log(`function show hand , card count: ${cards.length} v ${v}`);
    //console.log(cards);
    if(v===0){
        // اگر حاکم نبود
 /*        
 ...
 */
  ...
  function fun_hokm_is(val){
    ...
    val==3 ? hokm_icon='♦️' :0;
    val==4 ? hokm_icon='♣️' :0;
    console.log(`hokm is ${val} hokm_icon ${hokm_icon}`);
 ...
 // ست کردن نوبت 
  function fun_set_turn_logo(val){
    console.log(val);
    const id='turn-player-'+val;
    document.getElementById('turn-player-1').innerText='';
    document.getElementById('turn-player-2').innerText='';
    document.getElementById('turn-player-3').innerText='';
    document.getElementById('turn-player-4').innerText='';
    document.getElementById(id).innerText='🔴';
    //king-player-seat-4
  }
...
function fun_result_single_game(val){
  ...
  const coin = Number(localStorage.getItem('coinGame'));
  const wcoin = 10;
  const lcoin = -10 ;
  const logbox = val ==='win' ? '🏅' : '💤';
  const title = val ==='win' ? 'You Win!' : 'You Loose!';
  const message =  val ==='win' ? 'You played awesome.' : 'Try to win.';
  const expr =  val ==='win' ? 0 : 0;
  const point =  val ==='win' ? wcoin : lcoin;
  val ==='win' ? play_event_win() : play_event_loose();
  val ==='win' ? localStorage.setItem(`coin`, Number(localStorage.getItem('coin'))+wcoin) :localStorage.setItem(`coin`, Number(localStorage.getItem('coin'))+lcoin) ;
  fun_single_game_server(val);
  ...
  <div class="container col l6 s10  row " id="container">
            ...
            <div  class="row" id="end-game-box-third-row">
                ...
                <div class="center col s8 m6 l6 xl6" >
                   ...
                   <div class="center col s10 m10 l10 xl10 z-depth-1 " id="end-game-box-point-box-title-box">
                      <h6 class="center" id="end-game-box-point-box-title-box-ex">${trans('Experience')} <span id="end-game-box-point-box-exp-nuumber">${expr}</span></h6>
                   </div>
                   <a onClick ="fun_side_menu_go('earn-coin')">
                  ...
                  <a onClick ="fun_side_menu_go('share')">
        ...
        <a onClick="fun_side_menu_go('play')" >
  ...
  <div class=".show-on-large col l3 s1" id="box-right-down">
      ...
      `;
      gtag('event', 'single_gameEnd');
}

function fun_single_game_server(val){
  console.log('fun_single_game_server');
  const host= window.document.location.host.replace(/:.*/, '')
  let client = new Colyseus.Client(location.protocol.replace("http", "ws") + "//" + host + (location.port ? ':'+location.port : ''));
  client.create("chat"
  ,{create: true, name:'local', point:val,privacy:true ,game_type:`17000`}
  )
      .then(room => {
         ...
         // auth_user();
          ...
          room.send({act:'singleHokm',id:localStorage.getItem('id'),result:val} );
          setTimeout(function(){ room.leave(); }, 400);
